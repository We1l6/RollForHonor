...
struct RectanglePro : Rectangle
{
	...
	RectanglePro(Vector2 _centerPos, Vector2 _size,
		float _rotation,
		float _cornerRadius,
		Color _color)
		: Rectangle{ 
			_centerPos.x - _size.x / 2.0f,
			_centerPos.y - _size.y / 2.0f,
			_size.x, 
			_size.y
		},
		rotation(_rotation), 
...
struct TextPro {
	...
	// full constructor
    TextPro(const std::string& _text,
            Font _font,
            int _fontSize,
            Color _color,
            Vector2 _position,
...
namespace UIUtils {
	...
	inline void DrawRectangleProRounded(const RectanglePro& rect)
	{
		...
		float normalizedRadius = rect.cornerRadius / std::min(rect.width, rect.height);
		if (normalizedRadius < 0.0f) normalizedRadius = 0.0f;
		if (normalizedRadius > 1.0f) normalizedRadius = 1.0f;
	...
	inline void DrawTextPro(const TextPro& textPro)
	{
        DrawTextPro(
            ...
            textPro.origin,
            textPro.rotation,
            static_cast<float>(textPro.fontSize),
            textPro.spacing,
            textPro.color
        );
	}
